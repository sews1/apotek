...
export default function MonthlyReport({ auth, monthlySales, month, year }) {
    // Initialize daily data first
    const dailySales = {};
    ...
    const totalItemsSold = monthlySales.reduce((sum, sale) => sum + (sale.items?.length || 0), 0);
    const avgDailySales = (totalSales / Object.keys(dailySales).length).toFixed(2);
    const avgDailyTransactions = (totalTransactions / Object.keys(dailySales).length).toFixed(2);
    
    // Get unique products
    const uniqueProducts = new Set();
    monthlySales.forEach(sale => sale.items?.forEach(item => uniqueProducts.add(item.product_id)));

    // Combine transaction data
    const combinedData = monthlySales.flatMap(sale => {
    ...
    const exportExcel = () => {
        ...
        const wsStats = XLSX.utils.aoa_to_sheet([
            ['Laporan Bulanan - Statistik Utama'],
            ['Bulan', `${month}/${year}`],
            [''],
            ...
            ['Rata-rata Penjualan Harian', avgDailySales],
            ['Transaksi per Hari', avgDailyTransactions],
        ]);
        ...
        saveAs(new Blob([wbout], { type: 'application/octet-stream' }), `Laporan_Bulanan_${month}_${year}.xlsx`);
    };
    ...
    const exportPDF = () => {
        ...
        // Title & Statistics
        doc.setFontSize(16);
        doc.text(`Laporan Penjualan Bulan ${month}/${year}`, pageWidth / 2, 15, { align: 'center' });

        doc.setFontSize(10);
        doc.text(`Total Penjualan: Rp ${totalSales.toLocaleString('id-ID')}`, 14, 25);
        doc.text(`Total Transaksi: ${totalTransactions}`, 14, 30);
        doc.text(`Rata-rata per Hari: Rp ${avgDailySales.toLocaleString('id-ID')}`, 14, 35);
        ...
        doc.save(`Laporan_Bulanan_${month}_${year}.pdf`);
    };

    return (
        <Authenticated auth={auth} header={`Laporan Bulanan - ${month}/${year}`}>
            <Head title={`Laporan Bulanan ${month}/${year}`} />
            ...
            <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <StatBox 
                        ...
                        trend={avgDailySales > (totalSales/30) ? 'up' : 'down'}
                        trendText={`Rp ${avgDailySales.toLocaleString('id-ID')}/hari`}
                        color="blue"
                    />
                    <StatBox 
                        ...
                        trend={avgDailyTransactions > (totalTransactions/30) ? 'up' : 'down'}
                ...
                <SectionTable 
                    ...
                    description="Daftar lengkap semua transaksi bulan ini"
                >
                    <thead className="bg-gray-50">
                        <tr>
