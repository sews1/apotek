...
export default function MonthlyReport({ auth, monthlySales }) {
    // Fungsi export Excel
    const exportExcel = () => {
        const wsData = monthlySales.map(({ date, invoice, customer, total }) => ({
            Tanggal: date,
            Invoice: invoice,
            Pelanggan: customer,
            Total: total,
        ...
        saveAs(blob, 'laporan-penjualan-bulanan.xlsx');
    };
    ...
    const exportPDF = () => {
        ...
        doc.text('Laporan Penjualan Bulanan', 14, 16);

        const tableColumn = ['Tanggal', 'Invoice', 'Pelanggan', 'Total'];
        const tableRows = monthlySales.map(sale => [
            sale.date,
            sale.invoice,
            sale.customer,
    ...
    return (
        <Authenticated auth={auth} header="Laporan Penjualan Bulanan">
            <Head title="Laporan Penjualan Bulanan" />

            <div className="bg-white p-6 rounded shadow border border-gray-100">
                <h2 className="text-lg font-semibold mb-4">Laporan Penjualan Bulanan</h2>
